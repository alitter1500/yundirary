...
public class NoteTypeDao {
     ...
     * @return
     */
    public List<TbNoteType> findTypeListByUserId(Integer userId){
       // 1. 定义SQL语句
        String sql="select typeId,typeName,userId from tb_note_type where userId = ? ";
        //2. 设置参数列表
        ...
        params.add(userId);
        //3. 调用BaseDao的查询方法，返回集合
        List<TbNoteType> list=BaseDao.queryRows(sql,params,TbNoteType.class);
        //4. 返回集合
        return list;
    }
    ...
    public Integer checkTypeName(String typeName, Integer userId, String typeId) {
        ...
        params.add(typeName);
        // 执行查询操作
        TbNoteType noteType= (TbNoteType) BaseDao.queryRow(sql,params,TbNoteType.class);
        // 如果对象为空，表示可用
        if (noteType==null){
            return 1;
        }else {
