package com.note01.service;

import cn.hutool.core.util.StrUtil;
import com.note01.dao.NoteDao;
import com.note01.po.TbNote;
import com.note01.util.Page;
import com.note01.vo.NoteVo;
import com.note01.vo.ResultInfo;
...
public class NoteService {
     ...
     * @return
     */
    public ResultInfo<TbNote> addOrUpdate(String typeId, String title, String content,String noteId,String lon,String lat) {

        ResultInfo<TbNote> resultInfo=new ResultInfo<>();
     ...
     * @return
     */
    public Page<TbNote> findNoteListByPage(String pageNumStr, String pageSizeStr, Integer userId,String title,String date,String typeId) {
        //设置分页参数的默认值
        Integer pageNum=1;// 默认当前页是第一页
        Integer pageSize=5;// 默认每页显示5条数据
        // 1. 参数的非空校验 （如果参数为空，则设置参数）
        ...
        // 4. 如果总记录数大于0，调用Page类的带参构造，得到其他分页参数的值，返回Page对象
        Page<TbNote> page=new Page<>(pageNum,pageSize,count);

        // 得到数据库中分页查询的开始下标
        Integer index=(pageNum-1)*pageSize;

        // 5. 查询当前登录用户下当前页的数据列表，返回note集合
        List<TbNote> noteList=noteDao.findNoteListByPage(userId,index,pageSize,title,date,typeId);
     ...
     * @return
     */
    public TbNote findNoteById(String noteId) {
        // 1. 参数的非空判断
        if (StrUtil.isBlank(noteId)){
            return null;
        }
        // 2. 调用Dao层的查询，通过noteId查询note对象
        TbNote note=noteDao.findNoteById(noteId);
        // 3. 返回note对象
        return note;
    }
     ...
     * @return
     */
    public ResultInfo<List<TbNote>> queryNoteLonAndLat(Integer userId) {
        ResultInfo<List<TbNote>> resultInfo=new ResultInfo<>();

        //通过用户ID查询云记记录
        List<TbNote> noteList=noteDao.queryNoteList(userId);
